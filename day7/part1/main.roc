...
trySolvePuzzle : Str -> Result U32 [UnableToParse Str]
trySolvePuzzle = \rawInput ->
    ...
    |> Result.map (\actions -> actions |> List.walk init apply)
    |> Result.map (\model -> model.root |> obtainDiskObjectsByCriteria criteria)
...
expect
    expected = { currentPath: [], root: Dir { path: [], children: [Dir { path: ["a"], children: [] }] } }
    actually = init |> applyDirOrFileInfo (DirInfo "a")
    actually == expected

expect
    expected = { currentPath: ["a"], root: Dir { path: [], children: [Dir { path: ["a"], children: [Dir { path: ["a", "b"], children: [] }] }] } }
